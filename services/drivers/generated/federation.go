...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Driver":
		resolverName, err := entityResolverNameForDriver(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Driver": %w`, err)
		}
		switch resolverName {
		...
		case "findDriverByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findDriverByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindDriverByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Driver": %w`, err)
			}
...
func entityResolverNameForDriver(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Driver", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Driver", ErrTypeNotFound))
			break
		}
		return "findDriverByID", nil
	}
	return "", fmt.Errorf("%w for Driver due to %v", ErrTypeNotFound,
