...
func (ec *executionContext) resolveEntity(
	...
	case "Vehicle":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Vehicle": %w`, err)
		}
		switch resolverName {
		...
		case "findVehicleByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findVehicleByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindVehicleByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Vehicle": %w`, err)
			}
...
func entityResolverNameForVehicle(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Vehicle", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Vehicle", ErrTypeNotFound))
			break
		}
		return "findVehicleByID", nil
	}
	return "", fmt.Errorf("%w for Vehicle due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
