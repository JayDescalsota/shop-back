...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Staff":
		resolverName, err := entityResolverNameForStaff(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Staff": %w`, err)
		}
		switch resolverName {
		...
		case "findStaffByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findStaffByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindStaffByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Staff": %w`, err)
			}
...
func entityResolverNameForStaff(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Staff", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Staff", ErrTypeNotFound))
			break
		}
		return "findStaffByID", nil
	}
	return "", fmt.Errorf("%w for Staff due to %v", ErrTypeNotFound,
